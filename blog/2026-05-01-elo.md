...
現在清單裡已經有 345 部電影了，偶爾會突然想起來還有什麼漏網之魚就會加進來（漫威太多了暫時有點懶得加），加入新的電影後，會先依據我的打分原則評斷，小數點後的數字再大致給定那個級距裡的 1~9 。
...
export const InfiniteMovieTournament = () => {
  ...
  return (
    <div style={{ 
      ...
      <h3 style={{ color: 'var(--ifm-color-emphasis-700)', marginBottom: '20px' }}>
        已評分: {matchCount} 局
      </h3>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
        ...
        >
          {/* 強制清除預設邊框與內距，避免 Docusaurus 樣式干擾 */}
          <h3 style={{ margin: 0, border: 'none', padding: 0 }}>{currentMatch[0].title}</h3>
          <span style={{ fontSize: '1rem', color: 'var(--ifm-color-emphasis-600)' }}>★ {Number(currentMatch[0].score).toFixed(1)}</span>
        </button>

        <div style={{ fontWeight: 'bold', fontSize: '24px', color: 'var(--ifm-color-emphasis-400)', fontStyle: 'italic' }}>
          VS
        </div>
        ...
        >
          {/* 強制清除預設邊框與內距，避免 Docusaurus 樣式干擾 */}
          <h3 style={{ margin: 0, border: 'none', padding: 0 }}>{currentMatch[1].title}</h3>
          <span style={{ fontSize: '1rem', color: 'var(--ifm-color-emphasis-600)' }}>★ {Number(currentMatch[1].score).toFixed(1)}</span>
        </button>
      </div>
    </div>
  );
