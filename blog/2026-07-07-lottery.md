  ...
  const toggleSimulation = () => {
    if (isRunning) {
      ...
      if (budget > 0 && budget < 50) {
        alert(`❌ 餘額不足：大樂透每注為 50 元，您的預算 (${budget} 元) 不足購買 1 注！`);
        return;
      }
  ...
  return (
    <div style={{ 
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        />
        <small style={{ color: 'var(--ifm-color-emphasis-600)' }}>* 每注金額為 50 元。輸入 0 代表無限模式，直到中頭獎為止。</small>
      </div>
      ...
      <div style={{ textAlign: 'center', marginBottom: '20px' }}>
        <button 
          ...
          disabled={isJackpot}
          style={{ 
            backgroundColor: isJackpot ? 'var(--ifm-color-success)' : isRunning ? 'var(--ifm-color-danger)' : 'var(--ifm-color-primary)', 
            color: '#fff', 
            border: 'none', 
            padding: '10px 20px', 
            fontSize: '15px', 
        ...
        >
          {isJackpot ? '🎉 已中頭獎' : isRunning ? '暫停模擬' : '開始模擬'}
        </button>
        <button 
          onClick={buyFullMeal} 
          disabled={isJackpot}
      ...
      {displayState && (
        <div style={{ borderTop: '2px dashed var(--ifm-border-color)', paddingTop: '15px' }}>
          <p style={{ margin: '5px 0', fontSize: '1.05em' }}>
            <b>本期真實獎號：</b>
            <span style={{ color: 'var(--ifm-color-danger)', fontWeight: 'bold' }}>{displayState.winningList.join(', ')}</span> 
            {' '}| 特別號：
            <span style={{ color: 'var(--ifm-color-warning)', fontWeight: 'bold' }}>[{displayState.specialNumber}]</span>
          </p>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px', fontSize: '0.95em' }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--ifm-color-emphasis-200)', borderBottom: '2px solid var(--ifm-border-color)' }}>
                <th style={{ padding: '10px', textAlign: 'left', border: '1px solid var(--ifm-border-color)' }}>會計項目</th>
                <th style={{ padding: '10px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>數值 / 金額</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style={{ padding: '10px', border: '1px solid var(--ifm-border-color)' }}>實際成功投注注數</td>
                <td style={{ padding: '10px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>{displayState.count.toLocaleString()} 注</td>
              </tr>
              <tr>
                <td style={{ padding: '10px', border: '1px solid var(--ifm-border-color)' }}>總投注成本 (支出)</td>
                <td style={{ padding: '10px', textAlign: 'right', color: 'var(--ifm-color-danger)', fontWeight: 'bold', border: '1px solid var(--ifm-border-color)' }}>
                  -{displayState.totalSpent.toLocaleString()} 元
                </td>
              </tr>
              <tr>
                <td style={{ padding: '10px', border: '1px solid var(--ifm-border-color)' }}>中獎總彩金 (收入)</td>
                <td style={{ padding: '10px', textAlign: 'right', color: 'var(--ifm-color-success)', fontWeight: 'bold', border: '1px solid var(--ifm-border-color)' }}>
              ...
              <tr style={{ backgroundColor: 'var(--ifm-color-emphasis-100)', fontWeight: 'bold' }}>
                ...
                <td style={{ padding: '10px', textAlign: 'right', color: net > 0 ? 'var(--ifm-color-success)' : net < 0 ? 'var(--ifm-color-danger)' : 'inherit', border: '1px solid var(--ifm-border-color)', fontSize: '1.05em' }}>
                  {net > 0 ? '+' : ''}{net.toLocaleString()} 元
                </td>
              </tr>
              <tr>
                <td style={{ padding: '10px', border: '1px solid var(--ifm-border-color)' }}>投資報酬率 (ROI)</td>
                <td style={{ padding: '10px', textAlign: 'right', fontWeight: 'bold', border: '1px solid var(--ifm-border-color)', color: parseFloat(roi) > 0 ? 'var(--ifm-color-success)' : parseFloat(roi) < 0 ? 'var(--ifm-color-danger)' : 'inherit' }}>
                  {roi} %
                </td>
              </tr>
            </tbody>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9em' }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--ifm-color-emphasis-200)', borderBottom: '2px solid var(--ifm-border-color)' }}>
                <th style={{ padding: '8px', textAlign: 'left', border: '1px solid var(--ifm-border-color)' }}>獎項</th>
                <th style={{ padding: '8px', textAlign: 'left', border: '1px solid var(--ifm-border-color)' }}>中獎條件</th>
                <th style={{ padding: '8px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>單注獎金</th>
                <th style={{ padding: '8px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>您的中獎注數</th>
                <th style={{ padding: '8px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>小計彩金</th>
              </tr>
            </thead>
            <tbody>
              {Object.keys(PRIZES).map(k => {
                ...
                return (
                  <tr key={k} style={{ 
                    ...
                    <td style={{ padding: '8px', border: '1px solid var(--ifm-border-color)' }}>{k}</td>
                    <td style={{ padding: '8px', border: '1px solid var(--ifm-border-color)', color: 'var(--ifm-color-emphasis-600)' }}>{PRIZES[k].desc}</td>
                    <td style={{ padding: '8px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>${PRIZES[k].amt.toLocaleString()}</td>
                    <td style={{ padding: '8px', textAlign: 'right', border: '1px solid var(--ifm-border-color)', color: myCount > 0 ? 'var(--ifm-color-primary)' : 'inherit' }}>
                      {myCount.toLocaleString()} 次
                    </td>
                    <td style={{ padding: '8px', textAlign: 'right', border: '1px solid var(--ifm-border-color)' }}>${subtotal.toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
